-- =================================================================
-- SOFA-2 评分系统优化版本 (第二次运行 - 不覆盖现有表)
-- 参考SOFA1的简洁结构，实现SOFA2最新标准
-- 性能优化：充分利用系统资源，添加并行查询和内存优化
-- =================================================================

-- =================================================================
-- 性能优化配置 (在会话级别设置)
-- =================================================================
SET work_mem = '256MB';                    -- 增加排序和哈希操作内存
SET maintenance_work_mem = '512MB';        -- 增加维护操作内存
SET effective_cache_size = '8GB';          -- 告知系统缓存大小
SET random_page_cost = 1.1;                -- 优化SSD访问
SET max_parallel_workers_per_gather = 4;   -- 启用并行查询
SET parallel_tuple_cost = 100;             -- 降低并行执行门槛
SET parallel_setup_cost = 1000;            -- 降低并行设置成本
SET enable_partitionwise_join = on;        -- 启用分区连接
SET enable_partitionwise_aggregate = on;   -- 启用分区聚合
SET jit = off;                             -- 对大查询禁用JIT编译

-- 创建性能监控表（如果不存在）
CREATE TABLE IF NOT EXISTS sofa2_run_progress (
...
-- 插入开始记录
INSERT INTO sofa2_run_progress (stage, status, details)
VALUES ('SOFA2优化脚本开始', 'STARTING', '第二次运行，使用新表名 sofa2_scores_v2');
...
-- 创建新表并插入数据（不覆盖现有表）
-- =================================================================
DROP TABLE IF EXISTS mimiciv_derived.sofa2_scores_v2 CASCADE;

-- 插入进度更新
INSERT INTO sofa2_run_progress (stage, status, details)
VALUES ('创建最终表', 'IN_PROGRESS', '创建 sofa2_scores_v2 表');

CREATE TABLE mimiciv_derived.sofa2_scores_v2 AS
SELECT
    stay_id,
    hadm_id,
    subject_id,
...
-- 添加主键和索引以提高查询性能
ALTER TABLE mimiciv_derived.sofa2_scores_v2
ADD COLUMN sofa2_score_id SERIAL PRIMARY KEY;

-- 创建并行索引以提高查询性能
CREATE INDEX CONCURRENTLY idx_sofa2_v2_stay_id ON mimiciv_derived.sofa2_scores_v2(stay_id);
CREATE INDEX CONCURRENTLY idx_sofa2_v2_subject_id ON mimiciv_derived.sofa2_scores_v2(subject_id);
CREATE INDEX CONCURRENTLY idx_sofa2_v2_hadm_id ON mimiciv_derived.sofa2_scores_v2(hadm_id);
CREATE INDEX CONCURRENTLY idx_sofa2_v2_total_score ON mimiciv_derived.sofa2_scores_v2(sofa2_total);
CREATE INDEX CONCURRENTLY idx_sofa2_v2_hr ON mimiciv_derived.sofa2_scores_v2(hr);

-- 添加复合索引用于常见查询模式
CREATE INDEX CONCURRENTLY idx_sofa2_v2_stay_hr ON mimiciv_derived.sofa2_scores_v2(stay_id, hr);

-- 添加表注释
COMMENT ON TABLE mimiciv_derived.sofa2_scores_v2 IS 'SOFA2评分系统结果表V2，包含每小时评分和24小时窗口最差评分（第二次运行版本）';
COMMENT ON COLUMN mimiciv_derived.sofa2_scores_v2.stay_id IS 'ICU住院ID';
COMMENT ON COLUMN mimiciv_derived.sofa2_scores_v2.hadm_id IS '住院ID';
COMMENT ON COLUMN mimiciv_derived.sofa2_scores_v2.subject_id IS '患者ID';
COMMENT ON COLUMN mimiciv_derived.sofa2_scores_v2.hr IS 'ICU住院小时数';
COMMENT ON COLUMN mimiciv_derived.sofa2_scores_v2.brain IS '神经系统评分(0-4)';
COMMENT ON COLUMN mimiciv_derived.sofa2_scores_v2.respiratory IS '呼吸系统评分(0-4)';
COMMENT ON COLUMN mimiciv_derived.sofa2_scores_v2.cardiovascular IS '心血管系统评分(0-4)';
COMMENT ON COLUMN mimiciv_derived.sofa2_scores_v2.liver IS '肝脏系统评分(0-4)';
COMMENT ON COLUMN mimiciv_derived.sofa2_scores_v2.kidney IS '肾脏系统评分(0-4)';
COMMENT ON COLUMN mimiciv_derived.sofa2_scores_v2.hemostasis IS '凝血系统评分(0-4)';
COMMENT ON COLUMN mimiciv_derived.sofa2_scores_v2.sofa2_total IS 'SOFA2总分(0-24)';

-- 更新进度
INSERT INTO sofa2_run_progress (stage, status, details)
VALUES ('完成', 'COMPLETED', 'SOFA2评分表 sofa2_scores_v2 创建完成');

-- 显示创建结果统计
SELECT
    'SOFA2评分表V2创建完成' AS status,
    ...
    NOW() - (SELECT start_time FROM sofa2_run_progress WHERE stage = 'SOFA2优化脚本开始' ORDER BY run_id DESC LIMIT 1) AS total_runtime
