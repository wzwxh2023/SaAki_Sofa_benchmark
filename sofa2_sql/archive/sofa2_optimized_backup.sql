...
, pf_ratios_all AS (
    ...
    LEFT JOIN mimiciv_derived.ventilation vd
        ...
        AND vd.ventilation_status IN ('HFNC', 'InvasiveVent', 'NonInvasiveVent')  -- 修复：仅限SOFA2定义的高级呼吸支持
...
, sf_ratios_all AS (
    ...
    INNER JOIN fio2_raw fio2
        ...
        -- 修复：严格匹配同一时间点或最接近的时间点（5分钟窗口内）
        AND ABS(EXTRACT(EPOCH FROM (fio2.charttime - spo2.charttime))) <= 300  -- 5分钟窗口
    ...
    LEFT JOIN mimiciv_derived.ventilation vd
        ...
        AND vd.ventilation_status IN ('InvasiveVent', 'NonInvasiveVent', 'HFNC')  -- SOFA2定义的高级呼吸支持
...
-- =================================================================
-- CARDIOVASCULAR/心血管 (SOFA2标准：修复多巴胺分档逻辑)
-- =================================================================
, cardiovascular AS (
    SELECT
        co.stay_id,
        ...
        CASE
            ...
            -- 4d: 多巴胺单药 > 40 μg/kg/min (SOFA2特殊规则)
            ...
            -- 3c: 多巴胺单药 >20 至 ≤40 μg/kg/min (SOFA2特殊规则)
